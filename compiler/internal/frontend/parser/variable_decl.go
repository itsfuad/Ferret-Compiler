...
func parseTypeAnnotations(p *Parser) ([]ast.DataType, bool) {
	...
	for {
		...
		if !ok {
			token := p.peek()
			p.ctx.Reports.AddSyntaxError(p.fullPath, source.NewLocation(&token.Start, &token.End), report.MISSING_TYPE_NAME, report.PARSING_PHASE)
			return nil, false
		}
...
func parseInitializers(p *Parser) ([]ast.Expression, bool) {
	...
	if p.match(lexer.EQUALS_TOKEN) {
		...
		for {
			...
			if value == nil {
				token := p.peek()
				p.ctx.Reports.AddSyntaxError(p.fullPath, source.NewLocation(&token.Start, &token.End), "Expected value after '=', got invalid expression", report.PARSING_PHASE)
				return nil, false
			}
...
func parseVarDecl(p *Parser) ast.Statement {
	...
	if len(parsedTypes) == 0 {
		if len(values) == 0 {
			token := p.peek()
			p.ctx.Reports.AddError(p.fullPath, source.NewLocation(&token.Start, &token.End), "cannot infer types without initializers", report.PARSING_PHASE).AddHint("👈😃 Add initializers to the variables")
			return nil
		}
	}
